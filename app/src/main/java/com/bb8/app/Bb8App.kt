...
fun Bb8App(viewModel: Bb8ViewModel = viewModel()) {
    ...
    val sensorSample by viewModel.sensorSample.collectAsState()
    val sensorStreamingEnabled by viewModel.sensorStreamingEnabled.collectAsState()
    val savedMacros by viewModel.savedMacros.collectAsState()
    val macroRecording by viewModel.macroRecording.collectAsState()
    val macroPlaying by viewModel.macroPlaying.collectAsState()
        ...
        Bb8Background(modifier = Modifier.fillMaxSize()) {
            when {
                ...
                connectionState is ConnectionState.Connected -> DriveScreen(
                    ...
                    recordedStepCount = recordedSteps.size,
                    ...
                    onToggleSensorStreaming = viewModel::toggleSensorStreaming,
                    onToggleMacroRecord = viewModel::toggleMacroRecording,
                    onSaveRecordedMacro = viewModel::saveRecordedMacro,
                    onPlayMacro = viewModel::playMacro,
                    onUploadMacro = viewModel::uploadMacroToDevice,
                    onDeleteMacro = viewModel::deleteMacro,
                    onStopMacroPlayback = viewModel::stopMacroPlayback,
                    onAddSampleMacro = viewModel::addSampleSquareMacro,
                )
                else -> ScanScreen(
                    modifier = Modifier
                        .fillMaxSize()
