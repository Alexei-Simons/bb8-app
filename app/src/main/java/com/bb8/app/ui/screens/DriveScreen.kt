...
import com.bb8.app.ui.components.SensorStreamPanel
...
fun DriveScreen(
    ...
    sensorSample: SensorStreamSample,
    sensorStreamingEnabled: Boolean,
    ...
    Box(modifier = modifier.fillMaxSize()) {
        ...
        DriveExtrasPanel(
            ...
            sensorStreamingEnabled = sensorStreamingEnabled,
            ...
            onToggleSensorStreaming = onToggleSensorStreaming,
            onOpenMacroEditor = { showMacroEditor = true },
        )

        if (sensorStreamingEnabled) {
            SensorStreamPanel(
                sample = sensorSample,
                streamingEnabled = sensorStreamingEnabled,
            )
        }
        ...
        Box(
            ...
            Joystick(
                ...
                driveEnabled = !diagnosticsOnly && !patrolActive && !macroPlaying,
                onStickChange = { x, y ->
                    stickX = x
                    stickY = y
                    onDrive(x, y)
        ...
        if (showMacroEditor) {
            MacroEditorScreen(
                ...
                isPlaying = macroPlaying,
                recordedStepCount = recordedStepCount,
                onBack = { showMacroEditor = false },
                onToggleRecord = onToggleMacroRecord,
                onSaveRecording = onSaveRecordedMacro,
                onPlayMacro = onPlayMacro,
                onUploadMacro = onUploadMacro,
                onDeleteMacro = onDeleteMacro,
                onStopPlayback = onStopMacroPlayback,
                onAddSampleMacro = onAddSampleMacro,
            )
        }
    }
}
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(
                ...
                Text(
                    text = "LED · Boost · Patrol · Animations · Sensors · Macros",
            ...
            AnimatedVisibility(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        SecondaryActionButton(
                            text = if (sensorStreamingEnabled) "Sensors on" else "Sensors off",
                            onClick = onToggleSensorStreaming,
                            modifier = Modifier.weight(1f),
                        )
                        SecondaryActionButton(
                            text = if (macroRecording) "Recording…" else "Macros",
